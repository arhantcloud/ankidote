...
def _log_likelihood(
    ...
    total = 0.0
    for (a, b, c), correct in zip(params, responses):
...
def estimate_theta(
    ...
    *,
    ...
    ``prior`` (the survey-seeded warm start) is returned unchanged when there
    is no response yet, and is used to break ties for all-correct /
    all-incorrect response patterns, whose MLE is otherwise unbounded.
    """
    if not responses:
        return prior
    ...
    if all_same:
        ...
        direction = 1.0 if responses[0] else -1.0
